...
lm(total_amount ~ trip_distance + passenger_count - 1, data = df) # El -1 quita el intercepto
...
# subset data
df_s = df %>% subset(df,trip_distance<1000) %>%  subset(df, total_amount < 300)
...
lm(total_amount ~ trip_distance + passenger_count + as.factor(payment_type), data=df_s) %>% summary()
# as.factor() es la funcion que da para poder utilizar una variable categorica
...
# libreria felm o lfe para datos panel
