...
p_load(tidyverse,viridis,sf,leaflet,osmdata,ggsn,skimr,ggmap,tidycensus) # llamar y/o instalar librerias
...
leaflet() %>% addTiles() %>% 
addCircleMarkers(data=amenitys , weight=1 , col="green")
...
browseURL("https://maplink.global/blog/es/como-obtener-google-maps-api-key/")
...
ggmap(get_googlemap(center = c(lon = -78.47524493549128, lat = -0.17743936182761244),
                    zoom = 10, scale = 2, maptype ='satellite', color = 'color')) 
...
#====================================#
# [4.] Modelo de equilibrio espacial #
#====================================#
...
ggmap(get_googlemap(center = c(lon = -71.057083, lat = 42.361145),
      zoom = 12, scale = 2, maptype ='satellite', color = 'color')) +
geom_sf(data=boston, col="orange" , fill=NA , size=0.3 , inherit.aes = F) +
geom_sf(data=boston_cbd, col="red" , fill=NA , size=0.3 , inherit.aes = F)
...
ggsave("clase_12/output/map_distance_boston_cbd.pdf")

ggplot() + geom_sf(data=chicago , col="black" , aes(fill=dist_CBD)) + 
scale_fill_viridis(option="A" , alpha=0.9 , direction=-1 , name="Dist. CBD (miles)") +
geom_sf(data=chicago_cbd , col = "green" , size = 5) + theme_bw()
ggsave("clase_12/output/map_distance_chicago_cbd.pdf")
